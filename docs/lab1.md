...
/* P.S.: here we are defining a subclass, and using it as a class member. */
```
...
Поле структуры должно начинаться с байта, кратного его размеру, для этого между полями структуры компилятор может вставлять пропуски, т.к. менять местами поля по стандарту компилятор не имеет права.
...
- Определение методов можно делать вне тела класса (но объявление по прежнему должно быть внутри).
...
Компилятор создает все эти конструкторы, только если они не созданы явно. Если же определить хоть один конструктор самому - остальные придется определять тоже самому.

> **Деструкторы** - это блок инструкций, вызываемые при уничтожении объекта класса.
...
> **Список инициализации (initialization list)** - синтаксическая возможность
...
Переменные, помеченные ключевым словом `const`, не могут быть изменены во время работы программы, им можно задать только значение при инициализации. 

Методы класса, помеченные ключевым словом `const`, не могут менять поля класса, а также вызывать другие методы без модификатора `const`.

У константных объектов можно вызывать только константные методы. Это относится к константным экземплярам класса, константным ссылкам и указателям на экземпляры классов.
...
Для выполнения задания используйте написанные ранее классы и функции.
